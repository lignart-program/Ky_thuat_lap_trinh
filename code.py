...
def themThanhVien(Projects):
    ...
    if Chon == "1":
        ...
        if not os.path.exists(FileExcel):
            ...
            return
        SoLuong=0
        DongDau=True # Cờ đánh dấu dòng đầu tiên
        with open(FileExcel, 'r', encoding='utf-8') as F:
            for L in F:
                L=L.strip()
                if not L:
                    continue
                if DongDau: # Nếu là dòng đầu tiên (dòng tiêu đề)
                    DongDau=False # Hạ cờ xuống
                    continue # Bỏ qua không xử lý dòng này
    ...
    # (Phần nhánh Chon == "2" giữ nguyên không thay đổi)
    elif Chon == "2":
    ...
    if Chon == "1":
        ...
        with open(FileExcel, 'r', encoding='utf-8') as F:
            for L in F:
                ...
                if not L:
                    continue
                if DongDau: # Tự động bỏ qua dòng tiêu đề Tiếng Việt
                    DongDau=False
                    continue
...
def ghiNhanTimesheet(Projects):
    ...
    # (Phần nhánh Chon == "2" giữ nguyên)
        ...
        for Nk in P.NhatKyCong:
            ...
            if T: TongChiPhi+=Nk.SoGioLam * T.DonGia
        if TongChiPhi + (SoGio * TvHienTai.DonGia) > P.NganSach:
            print(f"Lỗi: THỦNG NGÂN SÁCH (Hụt {TongChiPhi + (SoGio * TvHienTai.DonGia) - P.NganSach:,.0f} VND)!")
            return
...
def baoCaoDuAn(Projects):
    ...
    if NganSachConLai >= 0:
        ...
        print(f"   => CẢNH BÁO: THỦNG NGÂN SÁCH (Hụt {-NganSachConLai:,.0f} VND)")
    print("-" * 50)
...
def main():
    ...
    while True:
        ...
        if LuaChon == "1":
            ...
            luuDuLieu(Projects)
            print("Đã lưu CSDL thành công. Tạm biệt!")
            break
        else:
